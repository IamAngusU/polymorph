...
def main(argv: list[str] | None = None) -> int:
    ...
    env.update(
        {
            ...
            # Concurrent spawn tests need bounded native-library fan-out. Without this,
            # every child may initialize one BLAS thread per logical CPU even though the
            # tested CSV/JSON/SQLite paths do not perform numeric matrix work.
    ...
    passed = (
        stable and len(results) == len(steps) and all(item.status == "passed" for item in results)
    )
