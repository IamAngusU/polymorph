"""Return learned review recommendations beside unchanged policy decisions."""
...
def _source_from_file(item: dict[str, Any], root: Path) -> Any:
    ...
    elif file_format in ("json", "json5"):
...
def handle(payload: Any, root: Path, ranker: Any) -> dict[str, Any]:
    ...
    request = lab_probe._object(payload, {"protocol", "request_id", "operation", "items"})
    if request.get("protocol") != PROTOCOL or request.get("operation") not in (
        "map",
        "parse_map",
    ):
        raise lab_probe.InvalidRequest("unsupported learning probe contract")
    request_id = lab_probe._text(request.get("request_id"), 128)
        ...
        output.append(
            {
                ...
                "operation_wall_ms": (time.perf_counter_ns() - started) / 1e6,
            }
        )
    return {
        "protocol": PROTOCOL,
