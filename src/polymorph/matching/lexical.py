...
def field_text(field: Mapping[str, Any]) -> str:
    """Only declared descriptors, never cell values, field IDs or expected labels."""
    name = field.get("name")
    ...
    if any(not isinstance(x, str) or len(x) > 128 for x in aliases):
...
def features(query: str, candidate: str) -> dict[int, float]:
    ...
    qt, ct = sorted(set(q.split()))[:24], sorted(set(c.split()))[:24]
    out: dict[int, float] = {}

    def add(name: str, value: float = 1.0) -> None:
        idx = _index(name)
        out[idx] = out.get(idx, 0.0) + value

    add("bias")
    add("exact", float(bool(q) and q == c))
    add("edit", SequenceMatcher(None, q, c, autojunk=False).ratio())
    union = set(qt) | set(ct)
    add("overlap", len(set(qt) & set(ct)) / max(1, len(union)))
    add("length", min(len(q), len(c)) / max(1, len(q), len(c)))
    for left in qt:
        for right in ct:
            add("pair:" + left + "\x1f" + right, 1 / math.sqrt(max(1, len(qt) * len(ct))))
    # Character overlap is language-agnostic and helps abbreviations without a token dictionary.
    for n in (2, 3):
        qs = {q[i : i + n] for i in range(max(0, len(q) - n + 1))}
        cs = {c[i : i + n] for i in range(max(0, len(c) - n + 1))}
        add("char" + str(n), len(qs & cs) / max(1, len(qs | cs)))
    return out
...
def dot(weights: Sequence[float], vector: Mapping[int, float]) -> float:
    return sum(weights[i] * value for i, value in vector.items())
...
class LearnedRanker:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.weights) != DIMENSIONS or any(
            type(x) not in (int, float) or not math.isfinite(x) or abs(x) > 1e4
    ...
    def load(cls, path: str | Path, *, expected_sha256: str) -> LearnedRanker:
        ...
        try:
            obj = json.loads(raw, object_pairs_hook=_pairs, parse_constant=_reject_constant)
        ...
        if (
            obj["format"] != FORMAT
            or obj["feature_version"] != FEATURE_VERSION
            or type(obj["dimensions"]) is not int
            or obj["dimensions"] != DIMENSIONS
        ):
        ...
        return cls(tuple(obj["weights"]), observed)
    ...
    def rank_fields(
        ...
        if len(candidates) > MAX_CANDIDATES:
            raise ValueError("ranker candidate budget exceeded")
        identities = [x.get("id") for x in candidates]
        if any(
            not isinstance(x, str) or not x or len(x) > 128 for x in identities
        ) or len(set(identities)) != len(identities):
            raise ValueError("candidate identities must be unique bounded strings")
        scores = self.scores(field_text(source), [field_text(x) for x in candidates])
        results = [
            {"target_field_id": identity, "rank_score": score, "authority": "advisory_only"}
            for identity, score in zip(identities, scores, strict=True)
        ]
        return sorted(results, key=lambda x: (-x["rank_score"], x["target_field_id"]))
